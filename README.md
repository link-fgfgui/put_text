...
**`GET /paste?text=...`** —— 给浏览器跨源页面用的 **GET 信标**。

浏览器从别的源 `fetch` 这个服务会被 CORS 拦下（响应里没有
`Access-Control-Allow-Origin`，也处理不了预检）。`<img>` 则不受 CORS 限制，
所以前端把文本拼进 query 当图片加载，请求能真实送达。

为了让前端还能判断结果，这个接口的响应分两种：

| 情况 | 响应 | 前端看到 |
|---|---|---|
| 上屏成功 | `200` + 1×1 透明 GIF（`Content-Type: image/gif`） | `onload` → 已上屏 |
| 上屏失败 / 文本为空 | `200`/`400` + 上面的 JSON | `onerror` → 没成功 |

用 `Accept` 头区分信标和其他客户端：浏览器 `<img>` 的 `Accept` 里有 `image/`，
所以 curl（`Accept: */*`）、地址栏、脚本一律拿到 JSON。
...
- **文本会进 URL**。浏览器历史、代理日志、服务端访问日志里都会留一份，
  别用它传敏感内容。
- **URL 有长度上限**。前端按 4000 字符编码长度切块分多次发送，
  单次请求的正文越长越容易撞上浏览器或中间层的限制。
...
- **服务没有任何鉴权**。默认只监听 `127.0.0.1`，别人碰不到；
  用 `--host 0.0.0.0` 开放给手机之后就不同了：同一局域网里的任何设备，
  以及你浏览器里打开的任何网页（GET 信标正是为此设计的，天然绕过 CORS），
  都能往你当前焦点的窗口里打字。只在可信网络里这么开。
